...
export const CAREER_POSTS: Post[] = [
  {
    ...
    subtitle:
      "AI agent context engineering · CLI tooling · agent-pipeline design",
    ...
    images: [],
    paragraphs: [
      "<strong>haus-workflow</strong> is a two-repository system I designed and maintain to solve a specific infrastructure problem in AI-assisted development: keeping coding agents (Claude Code) equipped with accurate, non-stale project context at scale, across many repositories, without manual config upkeep.",
      "The <strong>CLI</strong> runs a three-stage pipeline: <strong>scan → recommend → apply</strong>. The scanner walks a repo with <code>fast-glob</code>, filtering out sensitive paths, then runs a typed, data-driven detection registry (<code>DetectionRule[]</code>) to infer role, stack, package manager, and dependency signals — matching actual pinned majors rather than guessing, and reading workspace-member manifests so a monorepo is not judged by its root alone. Those signals are cross-referenced against the catalog manifest by a contract check, so scanner logic and catalog content cannot silently diverge. The recommender is a <strong>binary policy-gate engine</strong> with no confidence scores or fuzzy ranking: each catalog item passes hard include/exclude gates (unsupported stack, curation/risk tier, source trust, sensitive content, required role) before positive match signals determine eligibility, which keeps every recommendation auditable and explainable via a dedicated <code>explain-recommendation</code> command. The apply stage writes generated config under <strong>hash-based lockfile tracking</strong>: on every update it diffs the previous lockfile against the current manifest and only deletes items whose on-disk content still matches the recorded hash. Hand-edits survive untouched, every deleter backs up before it removes anything, and <code>haus undo</code> / <code>haus backups restore</code> walk it back.",
      "Beyond single repos, the CLI manages <strong>workspaces</strong>: it discovers sibling repositories, sets up shared context across members, and materializes real git worktrees per member — with hydration, reclamation, and a session-start hook that rehydrates a worktree automatically. <code>haus new</code> scaffolds new projects from a CLI-owned stack registry of pinned scaffolders, reporting partial success as its own outcome rather than faking a clean run. It also owns one pinned global Claude Code plugin, tracking every pin it has ever shipped so a machine on a stale pin is cleaned up automatically, and gating out catalog items the plugin already provides.",
      "The <strong>catalog</strong> is a separate, independently versioned repository holding the authoritative content: 158 items (skills, agents, rules, templates, configs) registered in a JSON-schema-validated manifest, with explicit provenance distinguishing first-party content from automated, script-synced imports of ~30 upstream open-source collections. Its manifest format governs install semantics — curated rules mirror their upstream path, skills and agents flatten to a basename — and a catalog declaring an unsupported format is refused before the first write. The catalog runs its own CI (schema/frontmatter validation, lint, format, license reporting, upstream-sync and fixture-sync bots) and its reader-facing pages are synced out to a published Docusaurus site. The CLI fetches content at runtime through a single recursive GitHub tree listing per sync, then caches locally, so the two repos ship on independent cadences without version lockstep. Cross-repo drift is caught by a behaviour-parity check against the catalog's own reference simulator, a contract-drift workflow, and a non-blocking live-catalog canary — all authenticated by a GitHub App rather than long-lived tokens.",
  ...
  {
    id: "atlas290",
    slug: "project-atlas290",
    category: "project",
    title: "Atlas 290 | Solo Project",
    subtitle: "Interactive atlas of Sweden · Open-data pipeline",
    ...
    images: [
      {
        src: "/posts/project/atlas290/1.png",
        alt: "Atlas 290 front page: a choropleth map of Sweden's 290 municipalities coloured by population, with the year slider at 2025",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/2.png",
        alt: "The dark theme, showing population change in 1990 on a diverging purple-to-orange scale",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/3.png",
        alt: "The bubble cartogram, with each municipality sized and coloured by median income in 2024",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/4.png",
        alt: "Malmö's profile: generated prose summarising its story, then one row per measure with value, rank and a sparkline",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/5.png",
        alt: "Malmö compared with Stockholm, side by side with overlaid histories and no verdict",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/6.png",
        alt: "The five places most like Malmö, and five auto-generated facts that each link into the view that proves them",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/7.png",
        alt: "The sortable table view: women's life expectancy per municipality with value and rank",
        variant: "width",
      },
      {
        src: "/posts/project/atlas290/8.png",
        alt: "Kiruna's profile on a phone",
        variant: "height",
      },
    ],
    paragraphs: [
      "<strong>Atlas 290</strong> is an interactive atlas of all 290 Swedish municipalities, built entirely from Statistics Sweden (SCB) open data. It covers 44 measures from 1968 to today (population, income, house prices, education, life expectancy, turnout and more), and it's meant to be explored rather than queried. You drag a year slider and the country changes colour, press play and the decades run as a short story, or morph the real map into a bubble cartogram where the 290 shapes travel into circles sized by the measure on screen. Click a municipality to open its profile, compare it with another, or read the same view as a plain sortable table.",
      "The repository holds two programs with a hard wall between them. <strong>The kitchen</strong> is an offline Node pipeline and the only code allowed to touch the network. It queries SCB's PxWeb API within its cell and rate limits and freezes every raw response into the repo with a SHA-256 in a provenance manifest (38 tables so far). It then repairs Sweden's administrative history: municipalities created after 1968, every Skåne code changing in 1997, and the random noise SCB has added to population figures since 2025. Before anything is written it rejects implausible jumps, wrong counts and unreachable map nodes. Its output, <strong>the pantry</strong>, is a set of committed JSON files that <strong>the site</strong> reads and nothing else: no server, no runtime API, no tracking. The only coupling between the two is a set of shared Zod schemas, so if the kitchen changes its output shape, the site stops compiling.",
      "Determinism is a hard requirement. Publishing twice from the same frozen responses gives byte-identical files, and CI fails on any diff, so the monthly scheduled refresh arrives as a readable pull request of changed numbers. The kitchen also computes what SCB doesn't publish: inflation-adjusted money, colour breaks fixed across every year (so a municipality only changes colour when its value does), a keyboard adjacency graph with hand-curated edges so the arrow keys never dead-end at Gotland, one Dorling bubble layout per measure, each proven reachable before it ships, a similarity metric that names the five places most like each municipality, and a facts engine that finds and phrases five surprising facts in both languages, each linking into the view that proves it.",
      'The site is React 19, TypeScript and Vite, with the map drawn as 290 SVG paths via D3, which is where most of the accessibility comes from. <strong>The URL is the state</strong>: measure, year, selection, comparison, view and language all live in the address bar, so every view is a link, and the build writes 580 pre-rendered municipality pages with their own titles and preview cards. The design deliberately avoids verdicts. There\'s no "higher is better" flag anywhere, a comparison says something like "higher on 12 of 33 measures" and declines to name a winner, and missing data renders as one of seven distinct absence states rather than a misleading zero. It\'s bilingual Swedish/English (a string added to one language fails the typecheck until the other has it too), ships light and dark themes with self-hosted subset fonts, and enforces a Content Security Policy that allows no third-party requests.',
      "I built it the way I'd want a production system run. I wrote the design and each increment's plan, AI coding agents carried out the plans, and every decision that changed along the way is written up as one of 24 decision records. CI runs 1,551 unit tests, a Playwright suite in Chromium, Firefox and WebKit with axe accessibility scans, and a Lighthouse performance budget, and deploys to Cloudflare Pages only when all of it is green. Releases and the changelog are generated from Conventional Commits, and the version number means one thing: a major bump only when a published URL stops working.",
    ],
    links: [
      { href: "https://atlas290.pages.dev/en/", icon: "globe" },
      { href: "https://github.com/aniisabihi/atlas290", icon: "github" },
    ],
    thumbnail: "/illustrations/atlas290.png",
